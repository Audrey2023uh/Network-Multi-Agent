...
\centering
\begin{tikzpicture}[
  box/.style={draw,rounded corners=2pt,align=center,font=\sffamily\footnotesize,inner sep=4pt,minimum height=0.85cm,minimum width=2.15cm},
  arr/.style={-{Latex},thick}
]
\node[box,fill=gray!12] (gen) {Generator\\+ constraints};
\node[box,right=0.55cm of gen,fill=blue!8] (db) {SQLite\\instance};
\node[box,right=0.55cm of db,fill=green!10] (lab) {Labels\\T1--T6};
\node[box,below=0.55cm of db,fill=orange!12] (aud) {Realism +\\leakage audits};
\draw[arr] (gen) -- (db);
\draw[arr] (db) -- (lab);
\draw[arr] (db) -- (aud);
\end{tikzpicture}
...
We evaluate six frozen SQLite instances: \texttt{v1.1.0-INST} and independent seeds 101, 202, 303, 404, and 505, as listed in Table~\ref{tab:seeds}. SHA-256 digests are published in \texttt{benchmark/INSTANCE\_CHECKSUMS.json}. Seed-robustness and publication-validation reports accompany the package. SQLite is authoritative for evaluation; CSV/Parquet exports are optional derivatives. Schema families are summarized in Table~\ref{tab:schema}.
\input{tables/tab_seeds_schema}
